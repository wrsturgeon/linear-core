...
Extract Constant new_name =>
  "let rec new_name' reserved name = if in_ name reserved then unbound' (underscored name) reserved else name in new_name'".
...
Lemma generate_acc_det {a1 a2} (Ea : Map.Eq a1 a2) {r1 r2} (Er : Map.Eq r1 r2) {n1 n2} (En : Map.Eq n1 n2)
  : Map.Eq (generate_acc a1 r1 n1) (generate_acc a2 r2 n2).
Proof.
  intros x y. unfold generate. unfold generate_acc. unfold Map.fold.
  repeat rewrite MapCore.fold_spec. apply Map.bindings_eq in En.
  ...
  subst. rename n2 into names. generalize dependent y. generalize dependent x. generalize dependent r2.
  generalize dependent r1. generalize dependent a2. generalize dependent a1.
  induction names as [| [k v] tail IH]; intros; cbn in *. { apply Ea. }
  apply IH. 2: { exact Er. } intros. repeat rewrite Map.find_overriding_add. rewrite Ea.
  erewrite new_name_det. { reflexivity. } cbn. intros. repeat rewrite Map.find_iff.
  repeat rewrite Map.find_overriding_union. destruct (Map.find_spec r1 k1).
  - apply Er in Y. apply Map.find_iff in Y. rewrite Y. reflexivity.
  - destruct (Map.find_spec r2 k1). { apply Er in Y. apply N in Y as []. }
    repeat rewrite <- Map.find_iff. fold (Map.domain a1). fold (Map.domain a2).
    repeat rewrite Map.find_domain. erewrite Map.in_eq. { reflexivity. } assumption.
Qed.

Lemma generate_det {r1 r2} (Er : Map.Eq r1 r2) {n1 n2} (En : Map.Eq n1 n2)
  : Map.Eq (generate r1 n1) (generate r2 n2).
Proof. apply generate_acc_det; try assumption. apply Map.eq_refl. Qed.
